resource "aws_eip" "eks_ngw_eip_1a" {
  ...
  tags = merge(var.tags, { Name = "${var.project_name}-eip-1a" })
}

resource "aws_eip" "eks_ngw_eip_1b" {
  domain = "vpc"

  tags = merge(var.tags, { Name = "${var.project_name}-eip-1b" })
}
...
resource "aws_nat_gateway" "eks_ngw_1a" {
  ...
  tags = merge(var.tags, { Name = "${var.project_name}-ngw-1a" })
}
...
resource "aws_nat_gateway" "eks_ngw_1b" {
  ...
  tags = merge(var.tags, { Name = "${var.project_name}-ngw-1b" })
}
...
resource "aws_route_table" "eks_private_route_table_1a" {
  ...
  tags = merge(var.tags, { Name = "${var.project_name}-priv-rtb-1a" })
}
...
resource "aws_route_table" "eks_private_route_table_1b" {
  ...
  tags = merge(var.tags, { Name = "${var.project_name}-priv-rtb-1b" })
}
